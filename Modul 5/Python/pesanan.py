    ...
    if os.path.exists("pesanan_restoran.docx"):
        ...
        for para in doc.paragraphs:
            ...
            if text.startswith("ID Pesanan:"):
                if pesanan:
                    if "image" not in pesanan:  # Tambahkan kunci default jika tidak ada
                        pesanan["image"] = ""
        ...
        if pesanan:  # Tambahkan elemen terakhir setelah iterasi
            if "image" not in pesanan:
                pesanan["image"] = ""
            pesanan_list.append(pesanan)
    return pesanan_list
...
def update_pesanan(pesanan_list):
    ...
    for pesanan in pesanan_list:
        if pesanan['id'] == id_pesanan:
            ...
            save_to_docx(pesanan_list)
            print(f"Status pesanan {id_pesanan} telah diubah menjadi {new_status}.")
            return
...
def delete_pesanan(pesanan_list):
    ...
    for pesanan in pesanan_list:
        if pesanan['id'] == id_pesanan:
            pesanan_list.remove(pesanan)  # Hapus pesanan
            save_to_docx(pesanan_list)
